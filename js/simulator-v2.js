...
async function loadGroups(){
    ...
    try{
        ...
        console.error(error);
        container.innerHTML =
        `<div class="error">Error Loading Groups</div>`;
    }
}
...
function renderBestThirds(){
    ...
    const container =
    document.getElementById("bestThirdsSection");

    const thirds = [];

    Object.keys(groupsData).forEach(group=>{

        if(selectedTeams[group]?.third){
            thirds.push({
                group,
                team: selectedTeams[group].third
            });
        }
...
function generateKnockout(){
    ...
    if(qualified.length !== 32){
        knockout.innerHTML = `
            <div class="error">
                <h3>Invalid Qualification</h3>
                <p>Teams: ${qualified.length}/32</p>
            </div>
        `;
        return;
    }
    ...
    /* =========================
       FIXED 16AVOS BRACKET
       (World Cup style stable layout)
    ========================= */
